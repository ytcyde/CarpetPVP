...
public class Tick2Command {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register(
                LiteralArgumentBuilder.<CommandSourceStack>literal("tick2")
                        .requires(commandSourceStack -> commandSourceStack.hasPermission(1))
                        .then(Commands.literal("query")
                                .executes(commandContext -> tickQuery(commandContext.getSource())))
                        .then(Commands.literal("rate").then(Commands.argument("rate", FloatArgumentType.floatArg(1.0F, MAX_TICKRATE))
                        ...
                        .then(Commands.literal("step")
                                ...
                                .then(Commands.argument("time", TimeArgument.time(1)).suggests((commandContext, suggestionsBuilder) -> SharedSuggestionProvider.suggest(new String[]{"1t", "1s"}, suggestionsBuilder))
    ...
    private static String nanosToMilisString(long l) {
        return String.format("%.1f", (float) l / TimeUtil.NANOSECONDS_PER_MILLISECOND);
    }
    ...
    private static int setTickingRate(CommandSourceStack commandSourceStack, float f) {
        ...
        commandSourceStack.sendSuccess(() ->
                Component.translatable("commands.tick.rate.success", string), true
        );
        return (int) f;
    }
    ...
    private static int tickQuery(CommandSourceStack commandSourceStack) {
        ...
        if (serverTickRateManager.isSprinting()) {
            commandSourceStack.sendSuccess(() ->
                    Component.translatable("commands.tick.status.sprinting"), false
            );
            commandSourceStack.sendSuccess(() ->
                    Component.translatable("commands.tick.query.rate.sprinting", string2, string), false
            );
        } else {
            if (serverTickRateManager.isFrozen()) {
                commandSourceStack.sendSuccess(() ->
                        Component.translatable("commands.tick.status.frozen"), false
                );
            } else if (serverTickRateManager.nanosecondsPerTick() < commandSourceStack.getServer().getAverageTickTimeNanos()) {
                commandSourceStack.sendSuccess(() ->
                        Component.translatable("commands.tick.status.lagging"), false
                );
            } else {
                commandSourceStack.sendSuccess(() ->
                        Component.translatable("commands.tick.status.running"), false
                );
            }

            String string3 = nanosToMilisString(serverTickRateManager.nanosecondsPerTick());
            commandSourceStack.sendSuccess(() ->
                    Component.translatable("commands.tick.query.rate.running", string2, string, string3), false
            );
        }
        ...
        commandSourceStack.sendSuccess(() ->
                Component.translatable("commands.tick.query.percentiles", string4, string5, string6, ls.length), false
        );
        return (int) f;
    }
    ...
    private static int sprint(CommandSourceStack commandSourceStack, int i) {
        ...
        commandSourceStack.sendSuccess(() ->
                Component.translatable("commands.tick.status.sprinting"), true
        );
        return 1;
    }
    ...
    private static int setFreeze(CommandSourceStack commandSourceStack, boolean bl) {
        ...
        if (bl) {
            commandSourceStack.sendSuccess(() ->
                    Component.translatable("commands.tick.status.frozen"), true
            );
        } else {
            commandSourceStack.sendSuccess(() ->
                    Component.translatable("commands.tick.status.running"), true
            );
        }
    ...
    private static int step(CommandSourceStack commandSourceStack, int i) {
        ...
        if (bl) {
            commandSourceStack.sendSuccess(() ->
                    Component.translatable("commands.tick.step.success", i), true
            );
        } else {
    ...
    private static int stopStepping(CommandSourceStack commandSourceStack) {
        ...
        if (bl) {
            commandSourceStack.sendSuccess(() ->
                    Component.translatable("commands.tick.step.stop.success"), true
            );
            return 1;
        } else {
